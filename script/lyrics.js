...
module.exports.run = async function ({ api, event, args }) {
    try {
        ...
        api.sendMessage("Finding lyrics, please wait...", event.threadID, async (err, info) => {
            if (err) {
                console.error(err);
            ...
            try {
                const response = await axios.get(`https://markdevs69-1efde24ed4ea.herokuapp.com/search/lyrics?q=${encodeURIComponent(query)}`);
